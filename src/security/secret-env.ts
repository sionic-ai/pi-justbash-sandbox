...
function toSet(names: readonly string[] | undefined): Set<string> {
 ...
 */
export function isSecretEnvName(name: string, options?: SecretEnvClassifierOptions): boolean {
  const upper = normalize(name);
  const deny = toSet(options?.deny);
  if (deny.has(upper)) return true;
 ...
 */
export function selectSecretEnv(
  env: NodeJS.ProcessEnv | Record<string, string | undefined>,
  options?: SecretEnvClassifierOptions,
): Map<string, string> {
...
export function stripSecretEnv(
  ...
  for (const [name, value] of Object.entries(env)) {
    if (typeof value !== "string") continue;
    if (isSecretEnvName(name, options)) continue;
    out[name] = value;
  }
  return out;
}
